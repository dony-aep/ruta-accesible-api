...
Ejecutada desde Swagger sobre la API corriendo en local con el seed cargado, siguiendo los
pasos del README. Cada endpoint tiene al menos un caso válido y uno inválido; la columna
Código Obtenido es el resultado real de la ejecución, no el esperado.

## Módulo de Reportes e IA
...
| `/api/Reportes` | `POST` | Inválido | `{ "usuario": "<70 caracteres>", "descripcion": "Prueba de validacion", "lugarId": 1 }` (Excede el MaxLength de 60) | 400 Bad Request | 400 Bad Request — `El usuario no puede exceder 60 caracteres` | Pass |
| `/api/Reportes` | `POST` | Inválido | `{ "usuario": "tester", "descripcion": "Prueba de validacion", "lugarId": 999 }` (Lugar inexistente) | 400 Bad Request | 400 Bad Request — `Lugar con ID 999 no encontrado` | Pass |
...
| `/api/Lugares/{id}` | `PUT` | Inválido | `id: 1`<br>`{ "nombre": "Parque", "tipo": "Parque", "zona": "Norte", "direccion": "Calle 1", "latitud": 120, "longitud": -74, "esServicioAlCiudadano": true }` (Latitud fuera del rango -90 a 90) | 400 Bad Request | 400 Bad Request — `La latitud debe estar entre -90 y 90` | Pass |
...
## Módulo de Tipos de Barrera y Estadísticas
